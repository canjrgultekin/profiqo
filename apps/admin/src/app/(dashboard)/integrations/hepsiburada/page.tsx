// Path: apps/admin/src/app/(dashboard)/integrations/hepsiburada/page.tsx
"use client";
...
export default function HepsiburadaIntegrationPage() {
  const [displayName, setDisplayName] = useState("Hepsiburada");
  ...
  useEffect(() => {
    let cancelled = false;
    const load = async () => {
      const res = await fetch("/api/integrations/hepsiburada/connection", { method: "GET", cache: "no-store" });
      ...
      setConnectionId(cid);
      setDisplayName(payload?.displayName ?? payload?.DisplayName ?? "Hepsiburada");
      setMerchantId(payload?.merchantId ?? payload?.MerchantId ?? payload?.externalAccountId ?? "");
      setUsername(""); setPassword("");
      append(`Mevcut Hepsiburada bağlantısı yüklendi. connectionId=${cid}`);
    };
    load();
  ...
  const connectOrUpdate = async () => {
    ...
    if (!merchantId.trim()) return append("HATA: Merchant ID zorunlu.");
    if (!hasExisting && (!username.trim() || !password.trim())) return append("HATA: Username/Password zorunlu.");
    if (hasExisting && (!username.trim() || !password.trim())) return append("HATA: Güncellemek için Username/Password girin.");
    const res = await fetch("/api/integrations/hepsiburada/connect", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ displayName, merchantId, username, password }) });
    const payload = await res.json().catch(() => null);
    if (!res.ok) return append(`HATA: ${payload?.message || JSON.stringify(payload)}`);
    const cid = payload?.connectionId ?? payload?.ConnectionId;
    setConnectionId(cid); setHasExisting(true); setUsername(""); setPassword("");
    append(`${hasExisting ? "Güncellendi" : "Bağlandı"}. connectionId=${cid}`);
  };

  const test = async () => {
    if (!connectionId) return;
    const res = await fetch("/api/integrations/hepsiburada/test", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ connectionId }) });
    const payload = await res.json().catch(() => null);
  ...
  const startSync = async () => {
    if (!connectionId) return;
    setBatchId(null); setBatch(null);
    const res = await fetch("/api/integrations/hepsiburada/sync/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ connectionId, pageSize, maxPages }) });
    const payload = await res.json().catch(() => null);
  ...
  return (
    <div className="p-4 sm:p-6">
      <div className="mb-6 flex items-center gap-3">
        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-purple-100 text-lg dark:bg-purple-900/20">🟣</div>
        <div>
          <h2 className="text-xl font-bold text-dark dark:text-white">Hepsiburada Entegrasyonu</h2>
      ...
      <div className="rounded-xl border border-stroke bg-white p-5 shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div>
            <label className="mb-1.5 block text-xs font-medium uppercase tracking-wider text-body-color dark:text-dark-6">Merchant ID</label>
            <input className={inputCls} value={merchantId} onChange={(e) => setMerchantId(e.target.value)} placeholder="Hepsiburada MerchantId (GUID)" />
          </div>
          <div>
            <label className="mb-1.5 block text-xs font-medium uppercase tracking-wider text-body-color dark:text-dark-6">Username {hasExisting ? "(güncelle)" : ""}</label>
            <input className={inputCls} value={username} onChange={(e) => setUsername(e.target.value)} placeholder="Entegrasyon kullanıcı adı" />
          </div>
          <div>
            <label className="mb-1.5 block text-xs font-medium uppercase tracking-wider text-body-color dark:text-dark-6">Password {hasExisting ? "(güncelle)" : ""}</label>
            <input type="password" className={inputCls} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Entegrasyon şifresi" />
          </div>
        </div>
        ...
        <div className="mt-5 flex flex-wrap items-center gap-3">
          ...
          <div className="ml-auto flex flex-wrap items-center gap-2">
            <div className="flex items-center gap-1">
              <span className="text-[10px] text-body-color dark:text-dark-6">Limit</span>
            ...
            <div className="flex items-center gap-1">
              ...
              <input type="number" value={maxPages} onChange={(e) => setMaxPages(Number(e.target.value || 100))} className="w-16 rounded-lg border border-stroke bg-transparent px-2 py-2 text-xs outline-none dark:border-dark-3 dark:text-white" />
            </div>
