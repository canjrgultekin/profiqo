// Path: apps/admin/src/app/(dashboard)/integrations/whatsapp/dispatch/page.tsx
"use client";
...
const statusConfig: Record<string, { label: string; color: string }> = {
  "1": { label: "Kuyrukta", color: "bg-blue-light-5 text-blue-dark dark:bg-blue/10 dark:text-blue-light" },
  "2": { label: "Çalışıyor", color: "bg-yellow-light-4 text-yellow-dark-2 dark:bg-yellow-dark/10 dark:text-yellow-light" },
  "3": { label: "Gönderildi", color: "bg-green-light-7 text-green-dark dark:bg-green/10 dark:text-green-light" },
  "4": { label: "Engellendi", color: "bg-orange-100 text-orange-700 dark:bg-orange-900/20 dark:text-orange-400" },
  "5": { label: "Başarısız", color: "bg-red-light-5 text-red-dark dark:bg-red/10 dark:text-red-light" },
};
...
function StatusBadge({ status }: { status: string }) {
  const cfg = statusConfig[status] || { label: status, color: "bg-gray-2 text-dark-5 dark:bg-dark-3 dark:text-dark-6" };
  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      <div className="mb-6 grid gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-stroke bg-white p-5 shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
          <h3 className="mb-4 flex items-center gap-2 text-base font-semibold text-dark dark:text-white">
            <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-blue-50 text-xs dark:bg-blue/10">📦</span>
