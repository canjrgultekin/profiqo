// Path: apps/admin/src/app/(dashboard)/integrations/whatsapp/templates/studio/page.tsx
"use client";
...
export default function WhatsappTemplateStudioPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (!res.ok) {
        append(payload?.message || "Şablon yüklenemedi.");
        return;
      }
  ...
  const saveDraft = async () => {
    ...
    const payload = await res.json().catch(() => null);
    if (!res.ok) return append(payload?.message || "Kaydetme başarısız.");
                  ...
                  <div key={idx} className="rounded border border-stroke p-3 dark:border-dark-3">
                    <div className="mb-2 flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="rounded bg-red-500/20 px-2 py-1 text-xs text-red-dark dark:text-red-light"
                      >
                        Remove
                      </button>
                    </div>
