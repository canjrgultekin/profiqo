// Path: apps/admin/src/app/(dashboard)/integrations/whatsapp/templates/page.tsx
"use client";
...
function statusBadge(s: string): { label: string; className: string } {
  ...
  return { label: s, className: "bg-gray-3 text-dark-4 dark:bg-dark-3 dark:text-dark-6" };
}
...
function categoryBadge(c: string): string {
  ...
  return "bg-gray-3 text-dark-4 dark:bg-dark-3 dark:text-dark-6";
}
...
export default function WhatsappTemplatesPage() {
  ...
  const load = async () => {
    ...
    if (!res.ok) {
      setErr(payload?.message || "Şablonlar yüklenemedi.");
      setItems([]);
      setLoading(false);
      return;
    }
