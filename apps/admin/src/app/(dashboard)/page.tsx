...
export default function DashboardPage() {
  ...
  const load = async () => {
    ...
    if (!oRes.ok) {
      setErr(oPayload?.message || "Genel bakış yüklenemedi.");
      return;
    }
    ...
    if (!cRes.ok) {
      setErr(cPayload?.message || "Kanal dağılımı yüklenemedi.");
      return;
    }
    ...
    return (
      <div className="p-4 sm:p-6">
        <div className="flex items-center gap-3 rounded-xl border border-red-200 bg-red-50 px-5 py-4 dark:border-red-900/40 dark:bg-red/10">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-red-light-5 dark:bg-red-900/40">
            <svg
  ...
  if (!overview || !channels) {
    return (
      <div className="p-4 sm:p-6">
        <div className="mb-6">
          <div className="h-8 w-48 animate-pulse rounded-lg bg-gray-3 dark:bg-dark-3" />
          <div className="mt-2 h-4 w-72 animate-pulse rounded bg-gray-3 dark:bg-dark-3" />
        </div>
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {[1, 2, 3, 4].map((i) => (
            ...
            >
              <div className="h-4 w-24 animate-pulse rounded bg-gray-3 dark:bg-dark-3" />
              <div className="mt-3 h-7 w-32 animate-pulse rounded bg-gray-3 dark:bg-dark-3" />
            </div>
          ))}
        </div>
      </div>
              ...
              >
                <defs>
                  <linearGradient id="colorGelir" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#0B8FD8" stopOpacity={0.3} />
                    <stop offset="95%" stopColor="#0B8FD8" stopOpacity={0} />
                  </linearGradient>
