// Path: apps/admin/src/app/(dashboard)/settings/users/page.tsx
"use client";
...
function statusBadge(status: string): { label: string; className: string } {
  ...
  return {
    label: status || "Pasif",
    className: "bg-gray-3 text-dark-4 dark:bg-dark-3 dark:text-dark-6",
  };
}
...
export default function TenantUsersPage() {
  ...
  const load = async () => {
    ...
    if (!res.ok) {
      setErr(payload?.message || "Kullanıcılar yüklenemedi.");
      setItems([]);
      return;
    }
  ...
  const disable = async (id: string) => {
    ...
    if (!res.ok) setErr("Kullanıcı devre dışı bırakılamadı.");
    await load();
  };
  ...
  const activate = async (id: string) => {
    ...
    if (!res.ok) setErr("Kullanıcı aktifleştirilemedi.");
    await load();
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {err && (
        <div className="flex items-center gap-3 rounded-xl border border-red/20 bg-red-light-5 px-5 py-4 dark:border-red-900/40 dark:bg-red/10">
                    ...
                    <td className="px-4 py-3.5 text-right">
                      {u.status === "Active" ? (
                        <button
                          ...
                          className="rounded-lg border border-red/20 px-3 py-1.5 text-xs font-medium text-red transition-colors hover:bg-red-light-5 dark:border-red/20 dark:hover:bg-red/10"
                        >
                          Devre Dışı
                        </button>
                      ) : (
                        <button
                          ...
                          className="rounded-lg border border-green/20 px-3 py-1.5 text-xs font-medium text-green transition-colors hover:bg-green-light-7 dark:border-green/20 dark:hover:bg-green/10"
                        >
                          Aktifleştir
                        </button>
                      )}
