// Path: apps/admin/src/app/api/integrations/shopify/sync/start/route.ts
...
async function safeReadJson(res: Response): Promise<any> {
  ...
  catch { return { message: "Backend returned non-JSON", raw: text.slice(0, 2000) }; }
}
...
export async function POST(req: Request) {
  ...
  const upstream = await fetch(`${backendBaseUrl()}/api/integrations/shopify/sync/start`, {
    method: "POST",
    headers: {
  ...
  return NextResponse.json(await safeReadJson(upstream), { status: upstream.status });
}