...
export async function POST(_req: Request, { params }: { params: Promise<{ groupKey: string }> }) {
  ...
  const upstream = await fetch(`${apiBaseUrl()}/api/customers/dedupe/suggestions/${encodeURIComponent(groupKey)}/approve`, {
    method: "POST",
    headers: {
