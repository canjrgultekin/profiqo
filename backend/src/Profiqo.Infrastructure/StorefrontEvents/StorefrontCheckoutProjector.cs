...
public sealed class StorefrontCheckoutProjector : IStorefrontCheckoutProjector
{
        ...
        CancellationToken ct)
    {
        ...
        {
            // Unique constraint (tenant+channel+providerOrderId) veya başka constraint.
...
LIMIT 1";
        ...
        CancellationToken ct)
    {
        ...
        // Lookup: provider-agnostic (source_provider filtresi yok)
        ...
        {
            // Race condition: concurrent pixel event aynı identity ile yeni customer yarattı.
            // ChangeTracker'ı temizle ve tekrar lookup yap.
            ...
            if (!string.IsNullOrWhiteSpace(emailHash))
                retryGuid = await FindCustomerIdByIdentityHashAsync(tenantId.Value, (short)IdentityType.Email, emailHash, ct);

            if (retryGuid is null && !string.IsNullOrWhiteSpace(phoneHash))
                retryGuid = await FindCustomerIdByIdentityHashAsync(tenantId.Value, (short)IdentityType.Phone, phoneHash, ct);

            if (retryGuid is not null)
                return retryGuid.Value;

            // Retry de bulamadıysa orijinal hatayı fırlat
            throw;
        }
    }
