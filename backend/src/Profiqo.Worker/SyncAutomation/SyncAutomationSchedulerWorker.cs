                ...
                try
                {
                    ...
                    await strategy.ExecuteAsync(async () =>
                    {
                        ...
                        foreach (var cid in connIds)
                        {
                            ...
                            else if (conn.ProviderType == ProviderType.Hepsiburada)
                            {
                                await jobs.CreateAsync(new IntegrationJobCreateRequest(batchId, tenantGuid, cid, IntegrationJobKind.HepsiburadaSyncOrders, rule.PageSize, rule.MaxPages), stoppingToken);
                            }
                            else if (conn.ProviderType == ProviderType.Shopify)
                            {
                                await jobs.CreateAsync(new IntegrationJobCreateRequest(batchId, tenantGuid, cid, IntegrationJobKind.ShopifySyncCustomers, rule.PageSize, rule.MaxPages), stoppingToken);
                                await jobs.CreateAsync(new IntegrationJobCreateRequest(batchId, tenantGuid, cid, IntegrationJobKind.ShopifySyncOrders, rule.PageSize, rule.MaxPages), stoppingToken);
                                await jobs.CreateAsync(new IntegrationJobCreateRequest(batchId, tenantGuid, cid, IntegrationJobKind.ShopifySyncProducts, rule.PageSize, rule.MaxPages), stoppingToken);
                            }
                        }
