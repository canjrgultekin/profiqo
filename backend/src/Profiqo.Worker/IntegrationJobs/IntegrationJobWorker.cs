...
using Profiqo.Application.Integrations.Hepsiburada;
using Profiqo.Application.Integrations.Shopify;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                var hepsiburada = scope.ServiceProvider.GetRequiredService<IHepsiburadaSyncProcessor>();
                var shopify = scope.ServiceProvider.GetRequiredService<IShopifySyncProcessor>();
                ...
                try
                {
                    ...
                    else if (job.Kind == IntegrationJobKind.IkasSyncProducts)
                        processed = await ikas.SyncProductsAsync(job.JobId, tenantId, job.ConnectionId, job.PageSize, job.MaxPages, stoppingToken);
                    ...
                    else if (job.Kind == IntegrationJobKind.HepsiburadaSyncOrders)
                        processed = await hepsiburada.SyncOrdersAsync(job.JobId, tenantId, job.ConnectionId, job.PageSize, job.MaxPages, stoppingToken);

                    else if (job.Kind == IntegrationJobKind.ShopifySyncCustomers)
                        processed = await shopify.SyncCustomersAsync(job.JobId, tenantId, job.ConnectionId, job.PageSize, job.MaxPages, stoppingToken);
                    else if (job.Kind == IntegrationJobKind.ShopifySyncOrders)
                        processed = await shopify.SyncOrdersAsync(job.JobId, tenantId, job.ConnectionId, job.PageSize, job.MaxPages, stoppingToken);
                    else if (job.Kind == IntegrationJobKind.ShopifySyncProducts)
                        processed = await shopify.SyncProductsAsync(job.JobId, tenantId, job.ConnectionId, job.PageSize, job.MaxPages, stoppingToken);
